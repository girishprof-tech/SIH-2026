...
export type Robot = { robot_id: string; position: Point; heading: Heading; robot_type: RobotType; state: RobotState; battery_pct: number; current_task_id: string | null; priority_score: number; last_updated_tick: number; path: Array<Point & { t: number }> }
export type JobType = 'fetch_item' | 'sort_batch' | 'audit_checkpoint'
export type JobRequest = { job_type: JobType; item_id?: string; zone?: string; urgency: number }
export type JobResponse = { job_type: JobType; robot_type: RobotType; task_id?: string | null; audit_id?: string | null; robot_id?: string | null; status: string; message: string }
