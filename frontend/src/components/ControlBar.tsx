import { Pause, Play, RotateCcw, Wifi, WifiOff, Radio, AlertTriangle, Sparkles } from 'lucide-react'
import type { SocketStatus } from '../hooks/useFleetSocket'

type Props = { running: boolean; tick: number; timestamp: number; socket: SocketStatus; skipped: number; chaos: boolean; loss: number; busy: boolean; onAction: (action: 'start' | 'pause' | 'reset') => void; onChaos: (enabled: boolean, loss: number) => void; onDemo: () => void }
export function ControlBar({ running, tick, timestamp, socket, skipped, chaos, loss, busy, onAction, onChaos, onDemo }: Props) {
  ...
  return <header className="topbar">
    ...
    <div className="top-controls">
      <div className="button-group"><button className="control-button primary" disabled={busy || running} onClick={() => onAction('start')}><Play size={14} fill="currentColor" /> START</button><button className="control-button" disabled={busy || !running} onClick={() => onAction('pause')}><Pause size={14} fill="currentColor" /> PAUSE</button><button className="icon-button" disabled={busy} aria-label="Reset simulation" onClick={() => onAction('reset')}><RotateCcw size={15} /></button><button className="control-button demo-button" disabled={busy} onClick={onDemo}><Sparkles size={14} /> DEMO SCENARIO</button></div>
